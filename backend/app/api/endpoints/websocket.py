...
MAX_INPUT_QUEUE_SIZE: int = 1024


async def authenticate_user(token: str) -> tuple[User | None, str | None]:
    try:
...
async def terminal_websocket(
    ...
    user, e2b_api_key = await wait_for_auth(websocket)
    if not user:
        await websocket.close(code=4001, reason="Authentication failed")
        return

    async with SandboxService(e2b_api_key) as sandbox_service:
